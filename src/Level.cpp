    ...
    : Object(pge)
{
	Map = "__1555555555555hi5555555555552__"
		  ...
		  "_______70jk+________+jk08_______"
...
void Level::Update(float fElapsedTime)
{
	...
	{
		visiblePowerPellet = !visiblePowerPellet;
		powerPelletFlickTime = 0.0f;
	}
}
...
bool Level::WallOnSide(const olc::vi2d& id)
{
...
void Level::Draw()
{
	for (int y = 0; y < levelHeight; y++)
	{
		for (int x = 0; x < levelWidth; x++)
		{
			...
			switch (TileID)
			{
			case '*': case 'X': if (visiblePowerPellet) FillCircle({ 184 + x * 16, 48 + y * 16 }, 8, olc::Pixel(255, 255, 155)); break;
	...
	blackRects->Draw(olc::vf2d(41.0f * 16.0f, 15.5f * 16.0f));
	blackRects->Draw(olc::vf2d(10.0f * 16.0f, 15.5f * 16.0f));
