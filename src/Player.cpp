...
void Player::Move(Level* level)
{
	...
	{			
		//From Left or Right to Up and Down
		if (cmp(newPos, (olc::vf2d)crossPos))
		{
			if (newDir.y == -1 && !level->WallOnSide(olc::vi2d(crossPos.x, crossPos.y - 1)))
			{
				newPos.x = (int)crossPos.x;
				direction = { 0, -1 };
			}
			else if (newDir.y == 1 && !level->WallOnSide(olc::vi2d(crossPos.x, crossPos.y + 1)))
			{
				newPos.x = (int)crossPos.x;
				direction = { 0, 1 };
			}

			if (newDir.x == -1 && !level->WallOnSide(olc::vi2d(crossPos.x - 1, crossPos.y)))
			{
				newPos.y = (int)crossPos.y;
				direction = { -1, 0 };
			}
			else if (newDir.x == 1 && !level->WallOnSide(olc::vi2d(crossPos.x + 1, crossPos.y)))
			{
				newPos.y = (int)crossPos.y;
				direction = { 1, 0 };
			}
